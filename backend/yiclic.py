...
"""## API key 設定方法
### Step 1 — 將 Google Maps(SERPAPI) 與 OpenAI API Key 寫入使用者家目錄下的 .zshrc 設定檔

### Step 2 — 在 .zshrc 中加入以下環境變數設定
> 注意：= 左右不能有空格

```
export SERPAPI_API_KEY="你的 Google Maps Key"
export GPT_API_KEY="你的 OpenAI API Key"
```
...
# Google map prompt 設定
# 設定：目標店家 (使用 place_id)
TARGET_SHOPS = [
    {"name": "林記豆漿店", "place_id": "ChIJZZYstakFbjQRgfM2o6Ibc78"},  # 替換成真實 place_id
    {"name": "天一香肉焿順", "place_id": "ChIJKybdGT5OXTQRKjrGMZ9BeFk"},
]
...
def process_all_reviews_limited(api_key, target_shops, max_reviews_per_shop=50, max_pages_per_shop=3):
    ...
    for shop in target_shops:
        ...
        print(f"\n--- 處理: {restaurant_name} (ID: {place_id}) ---")
        ...
        while True:
            ...
            if not reviews:
                print(f"⚠️ {restaurant_name} 沒有評論")
                break

            for review in reviews:
                if reviews_for_shop >= max_reviews_per_shop:
                    print(f"達到 {restaurant_name} 最大評論數量限制 ({max_reviews_per_shop})")
                    break
            ...
            if not next_token or page_count >= max_pages_per_shop:
                if page_count >= max_pages_per_shop:
                    print(f"達到 {restaurant_name} 最大頁數限制 ({max_pages_per_shop})")
                break
...
def run_pipeline(
    ...
    if collected_data:
        ...
        print(f"\n--- 成功！共 {len(collected_data)} 條評論已追加到 {output_filename} ---")
    else:
        print("\n⚠️ 沒有收集到任何有效的評論資料。")
    ...
    print(f"ML 過濾後，食物相關評論共 {len(df_food)} 條")

    # === Step 2: 從 df_food["snippet"] 裡面取出清理後的評論 ===
    all_reviews_list = []
    for snippet in df_food["snippet"]:
        clean_snippet = str(snippet).replace("\n", " ").strip()
        all_reviews_list.append(clean_snippet)

    print(f"✅ all_reviews_list 已建立，共 {len(all_reviews_list)} 條評論")

    # === Step 3: 轉成你原本使用的格式（編號 + join） ===
    all_reviews_content = "\n\n".join(
    ...
    scoring_criteria = """你是一個評論標註員
    任務：根據評論內容，標記食物評分(1~5 分，可含 0.5)
    要則：
    只評食物(不考慮價格、環境、服務)
    若評論有「打卡曬贈品、折扣、送小菜」等，評分 -0.5
    評分基準：
    1：極難吃
    2：難吃
    3：普通
    4：好吃
    5：非常好吃
    輸入格式：每個評論間隔兩個換行，沒有遇到兩個換行前為同一個評論
    輸出格式：
    只輸出分數(如：3.5)
    """
    ...
    train_prompt = (
        scoring_criteria
        + "\n------------------------------------------\n"
        + "現在請依照以上規則，幫我為底下的餐廳評論打分（0~5分）：\n"
        + all_reviews_content
    )

    print("=== train_prompt 前 1000 字 ===")
    ...
    for attempt in range(5):
        try:
            conversation = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system",
                     "content": "你是一位客觀的美食評論家，只回覆每則評論的分數，每行一個分數，不要多說其他文字。"},
            ...
            if "Rate limit" in str(e):
                print(f"⚠️ rate limit，等待重試... ({attempt+1}/5)")
            else:
                raise e
    ...
    for i in range(min(10, len(all_reviews_list))):
        print("\n---")
        print(f"評論：{all_reviews_list[i]}")
        print(f"GPT 給分：{all_scores[i] if i < len(all_scores) else ''}")
    ...
    # 直接依序寫入 manual（不使用 clean_indices）
    ...
    print(f"✅ GPT 標註的新評論分數已寫回 {output_file}")
